...
\begin{homeworkProblem}{- Backpropagation}
    ...
    \begin{itemize}
        ...
        With this, we have to consider two cases for the derivative $\partial \hat{o} / \partial z_2$. When $i = j$, we are differentiating the softmax function applied to $\hat{o}_i$ with respect to its own logit (the class $i$), that means, we can apply standard derivation with the quotient rule as follows
        ...
        Combining \eqref{7.2.2} and \eqref{7.2.3} into \eqref{7.2.1}, we get
        ...
        We can use this result to state the following expression 

        \[
            \frac{\partial L}{\partial \bm{z^{(2)}}} = \frac{\partial L}{\partial \bm{\hat{o}}}\frac{\partial \bm{\hat{o}}}{\partial \bm{z^{(2)}}} = \frac{\partial L}{\partial W_{\text{out}}} = a_1^T \cdot \frac{\partial L}{\partial z_2}
        \]
        \[
        \frac{\partial L}{\partial a_1} = W_{\text{out}}^T \cdot \frac{\partial L}{\partial z_2}
        \]
        \[
        \frac{\partial h}{\partial z_1} = \begin{cases} 
        1 & \text{for } z_1 > 0 \\
        0.01 & \text{for } z_1 \leq 0
        \end{cases}
        \]
        \[
        \frac{\partial L}{\partial W_{\text{hidden}}} = x^T \cdot (\frac{\partial L}{\partial a_1} \odot \frac{\partial h}{\partial z_1})
        \]
